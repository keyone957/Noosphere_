...
### [트러블 슈팅]
## [문제 상황]
추리 게임인 이 프로젝트는 비교적 짧은 동아리 기간인 4개월 내에 3개의 엔딩 루트를 구현해야 했습니다.
그러나 기획자가 스토리 보드 및 추리 게임에 필요한 분기들을 어떻게 빠르게 관리할지에 대한 기준이 없다는 점이 문제였습니다. 
또한 개발 도중 기획 수정이 발생할 경우 이를 어떻게 빠르게 게임에 반영해야 할지도 큰 고민이었습니다. 
이로 인해 분기 관리의 복잡도 증가 및 개발 속도 저하라는 문제 또한 발생 하였습니다.

## [해결 방법]
#### [구글 스프레드 시트를 이용한 게임 이벤트 관리]

구글 스프레드 시트를 이용해 각 게임에 필요한 이벤트를 규칙을 정하여 작성하도록 하여 게임에 반영되도록 하는 시스템을 구현.
또한 기획자가 변경한 내용들을 실시간으로 게임에 반영되도록 구현하였음.
